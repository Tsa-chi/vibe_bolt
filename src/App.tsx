...
function App() {
  ...
  const [manualShoppingList, setManualShoppingList] = useState<{id: string; name: string; checked: boolean; category: string}[]>([]);
  ...
  const toggleManualShoppingItem = (id: string) => {
    setManualShoppingList(prev => prev.map(item => 
      item.id === id ? { ...item, checked: !item.checked } : item
    ));
  };

  const removeManualShoppingItem = (id: string) => {
    setManualShoppingList(prev => prev.filter(item => item.id !== id));
  };

  const mergeShoppingLists = () => {
    const mergedItems = manualShoppingList.map(item => ({
      item: item.name,
      checked: item.checked,
      category: item.category
    }));
    setShoppingList(prev => [...prev, ...mergedItems]);
